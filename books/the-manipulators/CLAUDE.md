...
## What this is
A **genesis-from-seed** project: build the world, power-system, and outline from the dream
seed (`research/dream-seed.md`), then draft — using the **Best Seller Studio** pipeline.
An epic power-system saga; the magic rules ARE the plot, so canon below is binding.
...
## How to continue
1. `cd book/genesis/the-manipulators` and read `STATE.yaml` and `feedback/progress.md`.
...
- **Absorb = theft** — the original owner loses the power (zero-sum, predatory).
- **Gift = self-sacrifice** — Kevin gives a power AWAY to empower someone; he no longer has it.
- **Impermanence** — held/gifted powers decay unless maintained (ticking-clock economy).
- **Manipulators bound by a cosmic law of reality** — metaphysical, larger than the org.
- Characters: **Kevin Jones** (22, amnesiac, absorbs+gifts); **Jake Hobb** (blond, green eyes,
  flies via Kevin's gift); met 6 years ago, **Panama City**. Full canon → `STATE.yaml`.
- Scope: one complete standalone book (larger runway seeded, not required).

## Open author decisions (ask, don't invent)
- See `STATE.yaml` open_author_decisions (amnesia cause/purpose, Jake's watch, org name/theme).

## Status (update as you go)
- Seeded; canon locked. Next: architect builds foundation.md + outline.md.
