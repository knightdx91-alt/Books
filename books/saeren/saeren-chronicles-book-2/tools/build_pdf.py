#!/usr/bin/env python3
"""
Build a print-ready interior PDF of Book Two to IngramSpark interior specs.
...
Output: delivery/production/Saeren-Chronicles-Book-Two-6x9-interior-<rev>.pdf
...
OUT = os.path.join(ROOT, "delivery", "production",
                   f"Saeren-Chronicles-Book-Two-6x9-interior{_rev}.pdf")
...
def build(pad_to_even=False, head_map=None):
    ...
    story.append(Paragraph("Book Two: The Resistance", title_sub))
    ...
    story.append(Paragraph("The Saeren Chronicles will conclude in", fm_center))
    story.append(Spacer(1, 0.10*inch))
    ...
    story.append(Paragraph(
        "The severing is mended and the war is over, but the world Viridia "
        "remade does not yet know what it has become — and the cost she paid in "
        "the dark is only beginning to come due.", teaser_it))
    ...
    doc = BookDoc(OUT, pagesize=(TRIM_W, TRIM_H),
                  title="The Saeren Chronicles - Book Two: The Resistance",
