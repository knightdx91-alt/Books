# Chapter 13: Balance, Not Victory

<!-- Word count: ~4,950 | Structure: essayistic -> scene (the chapter OPENS in the reflective/argumentative register working the problem of balance, then descends into a scene at the physical Book One source-cavern). Function: CLARIFICATION (the real answer forms: not un-mending, not control, not defeating the Horrors, but BALANCE -- acknowledge the void as the necessary counterpart, contain the source's overflow, make the world sustainable) -- and per the LOCKED author decision she ATTEMPTS the Bella's-name thread and PAYS a cost. END HOOK: the cost of balance becomes clear, and it is terrible and personal. Voice matched to Ch.10-12 / Books One-Two. Source-cavern matches Bk2 Ch.18 prose (low unworked stone, pedestal, pool, the split symbol, the thread that runs DOWN). Bella-thread matches Bk2 Ch.18 ("it had no bottom"). Grief held inward -- the Ch.10 private break is closed; no third break; no one sees her cry. Void stays ABSTRACT, no entity, MotV connection NOT resolved. Em-dash <=4. Anaphora <=2 deliberate. -->
