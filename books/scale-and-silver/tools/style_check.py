#!/usr/bin/env python3
"""
A Bond of Scale and Silver — cross-manuscript style checker.
Calibrated to the Anne Bishop / Black Jewels voice comp (see voice-dna.md):
  em-dash <=10/ch, simile <=3.0/1k, adverbs ALLOWED (<=20/1k), no motif allowlist.
...
FINGERPRINT_PHRASES = [
    # This book builds its own as unconscious tics emerge (log in feedback/style-flags.md).
    # Saeren-specific fingerprints removed 2026-07-08 (different book/voice).
]
...
ALLOWLIST = [
    # EMPTY by design (2026-07-08). This book cut its deliberate motif phrases and
    # matches Bishop's voice = no repeated tag-phrases. Add a motif here ONLY if the
    # author deliberately introduces one; otherwise repeated n-grams are real echoes.
]
...
def scan():
    ...
    ap.add_argument("--max-simile", type=float, default=3.0,
    ...
    ap.add_argument("--max-emdash", type=int, default=10,
