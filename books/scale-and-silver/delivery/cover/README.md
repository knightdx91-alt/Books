...
## Status (2026-07-10)
- **Author name** — set: **Søren Stromberg**, the adult line's pen name (on the final
  front `front-cover-soren-stromberg.png`, regenerated by `compose_cover.py`). The older
  `front-cover-post-peleos*.png` files are kept only as art history — do not ship them.
